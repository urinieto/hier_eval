...
Nevertheless, it has been shown that even these segments tend to be organized in a hierarchical manner \cite{Peeters2009}, and even a large dataset of hierarchical human annotations is publicly available \cite{Smith2011}.
However, most algorithms only operate at a flat level, likely because (i) this task is already challenging at this level \cite{Skywalker2014} and (ii) even if an algorithm could estimate hierarchical segments, no method to evaluate them has been proposed.
...
In this work we present the Segment Hierarchy Agreement Grade (SHAG), a method to evaluate hierarchical segmentations that focuses on the boundaries of the segments exclusively (i.e. we do not measure the segment labels).
This proposed metric works at a frame-level (similar to \cite{Levy2008}, a score used to evaluate the labels of the segments), and it is based on a rank retrieval evaluation where each frame is ranked depending on whether it belongs to the \emph{correct} segment.
The correctness of the segment is evaluated across all the layers of the time partitions, thus resulting in a metric that can both evaluate hierarchical and flat boundaries (note that a flat segmentation is only a specific case of a hierarchical one, where the number of layers is one).
With SHAG we facilitate the evaluation of existing flat estimations against hierarchical annotations (like \cite{Smith2011}), and encourage new publications on richer hierarchical algorithms.
We show the behavior of SHAG with multiple synthetic and human examples, and compare the scores with the existing methods when possible.
