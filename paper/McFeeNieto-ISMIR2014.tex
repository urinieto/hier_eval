...
To start with, we compare two flat boundary annotations (i.e. only one layer of annotation), and see how SHAG behaves compared to the standard hit rate measure at 3 seconds and the media deviations.
The synthesized flat boundaries in Figure \ref{fig:flat-flat} aim to capture the situation where an algorithm only estimates --without any time deviations-- a subset of all the reference boundaries.
Analyzing the 
