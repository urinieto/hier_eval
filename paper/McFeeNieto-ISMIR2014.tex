...
Nevertheless, it has been shown that even these segments tend to be organized in a hierarchical manner~\cite{Peeters2009}, and even datasets of hierarchically-structured human annotations are now publicly available~\cite{Smith2011, Hargreaves2012}.
...
\section{Existing boundary evaluation}\label{sec:curr_meth}
...
information retrieval, $P$ and $R$ are combined into an $F$-measure by computing their
harmonic mean.

As a complement to boundary detection evaluation, boundary \emph{deviation} seeks to 
quantify the median absolute difference in timing between reference and the closest 
estimated boundary events, and vice versa~\cite{turnbull2007supervised}.
...
Aggregating over all sample $q$, we obtain the SHAG metric:
\begin{equation}
\shag(H_E) \defeq \frac{1}{n} \sum_q g(q ; H_E).\label{shagunder}
\end{equation}
% \Cref{shagunder} is interpretable as a measure of under-segmentation, as it will be
% low when $H_E$ fails to predict $i$ being more

Note this loss is equivalent to that evaluated by for subjective artist similarity~\cite{mcfee2011}.
The difference here is that the reference rankings are induced directly from ordinal 
data, and not subject to consistency errors.
...
In this section we discuss the behavior of SHAG by showing various synthetic examples, and comparing them against other existing methods when possible.
...
The hit rate scores (top right part of the table of Figure \ref{fig:flat-flat}) obtain a precision of 100\%, since all the estimated boundaries are also in the reference.
...
The median deviations, displayed on the bottom right of the table, are always 0 seconds (i.e. highest possible score) both when evaluating the Estimation against the Reference (E2R) and vice versa (R2E), since we did not add any time fluctuation in the boundaries.
...
Since the estimated boundaries are undersegmented compared to the reference ones, we obtain a low score for $\mathpzc{H}_u$, but a perfect score for $\mathpzc{H}_o$, since the estimation does not oversegment at any point.
...
For longer time windows, we see how the oversegmentation grade $\mathpzc{H}_o$ significantly decreases, while the undersegmented one $\mathpzc{H}_u$ increases.
This illustrates how \dots TODO: I do not know how to justify this.

\subsection{Hierarchical vs Flat}

Here we present two different examples of a hierarchical reference against two versions of flat estimations: large and small scale.
These situations may happen when we evaluate an algorithm that tends to undersegment (large scale) or oversegment  (small scale) against a track that is annotated hierarchically (e.g. any track of the SALAMI dataset).
...
As opposed to the previous example, the lower layer of the reference boundaries match exactly with the estimations, which results into perfect undersegmentation scores for the shorter time windows.
...
This illustrates how small time windows can yield spurious results, and therefore they should not be used in a real evaluation scenario.
...
The undersegmentation grade remains at 100\% with any window length as expected.
...
Up to this point we have seen SHAG examples on synthesized annotations in order to clearly show how our method behaves in a controlled scenario.
In this subsection, we aim to illustrate how SHAG reacts to actual musical pieces with real human annotations and algorithm estimations.

In Figure \ref{fig:SALAMI-SALAMI} we see the boundaries (top) and the results (bottom) of the evaluation of two human annotations extracted from the SALAMI dataset\footnote{This is the track number 636 of the SALAMI dataset}.
This dataset not only contains hierarchical annotations, but also more than one human annotation per track for the vast majority of the set.
As we can see on the top right plot, annotator 2 (used here as the estimation) provides three layers of annotations (the third layer only appears on the second to last frame) instead of the only two of annotator 1 (treated as the reference).
Moreover, annotator 2 tends to oversegment compared to the other annotator, and this is reflected in $\mathpzc{H}_o$, especially when $w=30$ seconds.
Note that when the time window is the whole duration of the track (i.e. $w = \infty$), the oversegmentation score obtains a better grade, since across the entire track the scores average across all the frames and they tend to over estimate the results when they are sufficiently similar.
This is not desirable, since qualitatively the amount of oversegmentation of the estimation should be greater than the undersegmentation, which shows how it is not always optimal to use a time window covering the entire piece.
The undersegmentation scores remain greater or equal than the ones of the oversegmentation, since the reference is clearly undersegmented compared to the estimation.
...
Finally, in this last example we use the same annotator 1 of the SALAMI track 636 as the reference, but we use as the estimation the output of one of the few published boundary algorithms that is capable of estimating hierarchical boundaries: Ordinal Linear Discriminative Analysis (OLDA) \cite{McFee2014}\footnote{The source code can be found in: https://github.com/bmcfee/olda}.
This automatic estimation returns 17 layers of boundaries, which is clearly oversegmented with respect to the reference --- which has only 2 layers.
This results in an over penalization of the oversegmentation metric when using a time window that comprises the entire track ($\mathpzc{H}_o = 16.39\%$).
Moreover, we see how unreliable SHAG can be when using a short time window, where the undersegmentation score is really slow and the oversegmentation grade is 100\%.
The undersegmentation grade is also low when using the proposed time window (i.e. $w=30$ seconds), since even if the estimation contains many more layers than the reference, the segments tend to be much larger than the ones in the reference.
...
The presentation of $\mathpzc{H}_u$ and $\mathpzc{H}_o$ should shed enough light to researchers in order to successfully evaluate and compare various hierarchical boundary algorithms.
...
Compact the SHAG measure using the F-measure as in OLDA\cite{McFee2014} and 2D-FMC \cite{Nieto2014}.
