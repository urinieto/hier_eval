...
For larger $w$, $\shag_P$ decreases, while $\shag_R$ increases, obtaining a very large boost as $w$ approaches the length of the track.
As suggested by these results, a sweet spot for $w$ might be around 15 seconds in this specific example.
This will be further explored in Section~\ref{subsec:human-annotator}.
...
Furthermore, due to the depth of the estimated boundaries, the Full results are inflated compared to the Reduced ones.
...
$1$ on 726 tracks from SALAMI\@, and using three different window settings and the reduced type.
