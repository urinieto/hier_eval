...
\begin{align}
f(q ; S_E, S_R) &\defeq &
\sum_{\substack{i \in S_R(q) \setminus \{q\},\\ j \notin S_R(q)}}
\frac{\ind{S_E(q) = S_E(i) \neq S_E(j) }}{Z_q} \notag\\
Z_q &\defeq& (|S_R(q)| - 1)\cdot (n - |S_R(q)| + 1), \label{flatrecall}
\end{align}
...
\begin{equation}
g(q ; H_E, H_R) \defeq \sum_{\substack{(i, j),\\ i \neq q,\\ H_R(q, i) \prec H_R(q, j)}}
...
albeit at the level of frame triplets rather than boundaries.  Finally, an $F$-measure can be defined in the usual way
...
\begin{equation}
  g(q ; H_E, H_R, m) \defeq \sum_{\substack{
  m_s \leq i,j \leq m_e \\ 
