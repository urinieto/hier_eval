...
We will assume that sample frames $X$ are generated from the track at some fixed resolution $f_r$ (\eg, 10Hz), although non-uniform samplings (\eg, beat- or onset-aligned samples) are easily accommodated.  
$S$ therefore is a disjoint partition of $X$, and $S(i)$ will identify the segment containing the $i$th frame.
...
respectively.
From the figure, we observe that $H(j,k)$ refers to the least common ancestor of frames $j$ and $k$.
We can generally infer membership and precedence relations, such as
\begin{equation}
...
\Cref{flatrecall} is defined in terms of segment membership (in)equalities, but we can generalize this notion 
to express the function using strict precedences in a hierarchy.
