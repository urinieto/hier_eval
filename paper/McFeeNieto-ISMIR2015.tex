...
% ---------------
%\oneauthor
% {Brian McFee, Oriol Nieto, and Juan Pablo Bello}
% {Music and Audio Research Lab\\ New York University\\ {\tt \{brian.mcfee, oriol, jpbello\}@nyu.edu}}
...
\def\authorname{Brian McFee, Oriol Nieto, Juan P. Bello}
...
% --------------
%\threeauthors
  %{Brian McFee} { \\ {\tt author1@ismir.edu}}
  %{Oriol Nieto} {\bf Retain these fake authors in\\\bf submission to preserve the formatting}
  %{Juan Pablo Bello} {Affiliation3 \\ {\tt author3@ismir.edu}}
...
The analysis of structure in music is a principal area of interest to musicologists.
Its goal is to identify and characterize the form of a musical piece by investigating the organization of its components, such as sections, phrases, melodies, or recurring motives.
...
and can be modeled and analyzed using tree representations~\cite{Lerdahl1983}.
...
The segmentation task has historically been geared toward algorithms which produce
a flat partition of the recording into disjoint segments.
...
and the annotation is not required to cover the entire piece.
...
Although not all music can necessarily be modeled using trees~\cite{Wiggins2015}, we argue that tree-based evaluation represents 
a first step toward moving beyond flat structure analyses.
...
each corresponding to different levels of analysis~\cite{Smith2011}.

When only the estimation is flat, it is still not obvious how to compute accuracy against multiple layers.
Aggregating reference boundaries across layers prior to evaluation would imply 
...
their contribution to the total score may be diluted by the abundance of low-level boundaries, which necessarily
outnumber high-level boundaries in hierarchical annotations.

Flat evaluation followed by aggregation across layers can be similarly problematic, 
...
--- \eg, SALAMI's \emph{large}- and \emph{small}-scale annotations --- it is unclear how to proceed in
...
\begin{figure}
  ...
  Frames $i, j$, and $k$ are indicated along the $x$-axis, and their containing segments are indicated
...
\Cref{flatrecall} is defined in terms of segment membership equality, but it has a straightforward 
...
Second, for large values of $n$ (long tracks), \Cref{hierrecall} can be dominated by trivial comparisons
...
Finally, the calculation of \Cref{shagunder} can be expensive, taking
...
%We show the two grades of $T$-measures (under-segmentation, over-segmentation) for all the examples in this sections
...
This section is subdivided by the types of annotations under consideration.
...
duration.
%As suggested by these results, a sweet spot for $w$ might be around 15 seconds in this specific example.
The dependency on $w$ is further explored in \Cref{subsec:human-annotator}.
...
\begin{figure}
  ...
  (bottom).}\label{fig:hier-flatlarger}
\end{figure}
...
\begin{figure}[t]
  ...
  \begin{minipage}{\columnwidth}
    ...
    \end{tabular}
  \end{minipage}
  \caption{Hierarchical reference vs.\ flat, small-scale estimation (top) and $T$-measures (bottom).}\label{fig:hier-flatsmall}
\end{figure}
...
\begin{figure}[t]
  ...
  \begin{minipage}{\columnwidth}
    ...
    \end{tabular}
  \end{minipage}
  \caption{Hierarchical reference vs.\ flat over-segmentation (top) and $T$-measures (bottom).}\label{fig:hier-flatsmaller}
\end{figure}
...
% (even if this new segment creates a new layer it still over-segments).
...
\begin{figure}[t]
  ...
  \begin{minipage}{\columnwidth}
    ...
    \end{tabular}
  \end{minipage}
  \caption{2-layer vs. 3-layer hierarchical boundaries (top) and $T$-measures scores (bottom).}\label{fig:hier-hiercomp}
\end{figure}
...
\subsection{Human annotator agreement}\label{subsec:human-annotator}
...
\begin{figure}[t]
  ...
  annotators. Top: annotations; bottom: $T$-measures scores.}\label{fig:SALAMI-SALAMI}
\end{figure}
...
trend.} 
The example in \Cref{fig:SALAMI-SALAMI} 
...
\Cref{fig:SALAMI-OLDA} illustrates one example output of OLDA and the resulting
$T$-measures.
...
\Cref{fig:salami-olda-stats} displays the $\shag_F$ score distribution for OLDA, measured
